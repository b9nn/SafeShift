    ...
    -- ============================================================================
    -- Drop share first so CREATE OR REPLACE DATABASE can succeed (shared DB cannot be dropped)
    DROP SHARE IF EXISTS SAFESHIFT_COMPLIANCE_SHARE;
    ...
    AS
        ...
        UNION ALL
        SELECT 'SML2010', DATEADD('SECOND', epoch_time, '1970-01-01 00:00:00'::TIMESTAMP_NTZ), co2_comedor, NULL, NULL,
    ...
    CREATE OR REPLACE VIEW MV_WEEKLY_COMPLIANCE_TREND AS
        SELECT
            factory_id,
    ...
    CREATE OR REPLACE VIEW V_ANOMALY_TRAINING_DATA AS
        SELECT reading_timestamp, factory_id, co2_ppm, temperature_c, humidity_pct, pressure_kpa,
    ...
    CREATE OR REPLACE VIEW V_CONTRIBUTION_DATA AS
        SELECT compliance_date, dc.factory_id, avg_compliance_score AS metric,
    ...
    -- Uncomment and replace with your Snowflake user email(s). Go to Admin > Users to verify.
    /*
    CREATE OR REPLACE NOTIFICATION INTEGRATION safeshift_alerts
        TYPE = EMAIL
        ENABLED = TRUE
        ALLOWED_RECIPIENTS = ('your-snowflake-email@example.com');
    ...
    CREATE OR REPLACE ALERT ALERT_CRITICAL_RISK
        ...
        THEN CALL SYSTEM$SEND_EMAIL('safeshift_alerts','your-snowflake-email@example.com',
