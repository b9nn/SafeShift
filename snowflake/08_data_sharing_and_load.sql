...
-- Re-set database context (CREATE ROLE is account-level and can clear session DB)
USE DATABASE SAFE_SHIFT;
...
GRANT SELECT ON VIEW SAFE_SHIFT.ANALYTICS.SV_PUBLIC_COMPLIANCE_SUMMARY
    TO SHARE SAFESHIFT_COMPLIANCE_SHARE;
GRANT SELECT ON VIEW SAFE_SHIFT.ANALYTICS.SV_BLOCKCHAIN_VERIFICATION
    TO SHARE SAFESHIFT_COMPLIANCE_SHARE;
...
CALL ANALYTICS.SP_LOAD_SAFETY_THRESHOLDS(
...
-- Check for load errors (only works immediately after a successful COPY INTO;
-- comment out if no files have been staged yet)
